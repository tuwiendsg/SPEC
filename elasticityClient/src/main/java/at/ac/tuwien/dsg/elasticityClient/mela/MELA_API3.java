 ...
 * the License.
 */
package at.ac.tuwien.dsg.elasticityClient.mela;
...
import at.ac.tuwien.dsg.spec.rsyblclient.utils.ClientLogger;
import at.ac.tuwien.dsg.spec.rsyblclient.utils.Configuration;

import java.io.StringWriter;

public class MELA_API3  {
...
//    }
    ...
    public void refreshMonitoringData() {
        ...
        try {
            ...
            ClientLogger.logger.error("Failing to connect to MELA");
    ...
    public void submitServiceConfiguration(Node cloudService) {
        ...
        while (notConnected) {
            try {
                ClientLogger.logger.info("Trying to connect to MELA ...");
                ...
                ClientLogger.logger.info(stringWriter.toString());
                os.flush();
                os.close();
                ...
                ClientLogger.logger.error("Failing to connect to MELA" + e.getMessage());
                try {
...
//    //should at least have an action name in name
    ...
    public Double getMetricValue(Metric metric, Node entity) {
        ...
        ClientLogger.logger.info("Metric " + metric.toString() + " OR   Node " + entity.getId() + " not found");
        return -1.0;
    }
        ...
        for (ElasticityRequirement elasticityRequirement : description) {
           // RuntimeLogger.logger.info("Setting elasticity requirement " + elasticityRequirement.getAnnotation().getConstraints());
            ...
            for (Constraint constraint : specification.getConstraint()) {
                ClientLogger.logger.info("Transformed constraint " + elasticityRequirement.getAnnotation().getConstraints());
                ...
                for (BinaryRestrictionsConjunction binaryRestrictions : constraint.getToEnforce().getBinaryRestriction()) {
                    for (BinaryRestriction binaryRestriction : binaryRestrictions.getBinaryRestrictions()) {
                        ...
                        ClientLogger.logger.info("Requirements for MELA" + req.toString());
    ...
    public Double getCpuUsage(Node entity) {
        Metric metric = new Metric("cpu_usage");
        ClientLogger.logger.info("For entity " + entity.getId() + " Cpu usage is " + getMetricValue(metric, entity) + "cpu idle is" + getMetricValue(new Metric("cpu_idle"), entity));
        return getMetricValue(metric, entity);
    }
    ...
    public void submitCompositionRules() {
        try {
            ...
            try {
                ...
                ClientLogger.logger.error("Submitting file composition rules. Error when submitting composition rules, in MELA_API" + e.getMessage());
    ...
    public void submitCompositionRules(String compositionRules) {
        try {
            ClientLogger.logger.info("In MELA_API: setting composition rules = " + compositionRules);
            Unmarshaller unmarshaller = JAXBContext.newInstance(CompositionRulesConfiguration.class).createUnmarshaller();
            ...
            ClientLogger.logger.error("Error when submitting composition rules, in MELA_API " + e.getMessage());
